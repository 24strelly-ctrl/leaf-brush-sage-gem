...
import { Pool } from "pg";
import { ensureDbReady, getPglite, getSqlite, dbSource as getDbSource } from "../db";
...
// Real Postgres when `DATABASE_URL` is set (deployed apps), Supabase when configured,
// else the app's embedded PGLite (preview) or SQLite via a Kysely dialect — so Better Auth persists to the
...
const database = databaseUrl
  ...
  : supabaseUrl
    ? new Pool({ 
        connectionString: `${supabaseUrl}?pgbouncer=true`,
        password: supabaseServiceKey
      })
    : getDbSource === "sqlite"
      ? { dialect: sqliteDialect(() => getSqlite()), type: "sqlite" as const }
